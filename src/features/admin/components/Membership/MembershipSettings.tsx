...
export const allAccessCategories: AccessCategory[] = [
	"Student",
	"AHEC Affiliate",
	"GT34",
	"Senior",
...
const MembershipSettings = ({ onComplete }: MembershipSettingsProps = {}) => {
	...
	const handleFormSubmit = (values: FormValues) => {
		const newMembership: MembershipType = {
			...
			name: values.name,
			accessCategories: values.categories.map(
				(category: { name: AccessCategory }) => category.name
			),
		...
		if (onComplete) onComplete(); // Call onComplete after successful submission
	};
	...
	const handleDelete = (id: string) => {
		if (
			...
			if (onComplete) onComplete(); // Call onComplete after successful deletion
		}
	};
							...
							>
								...
								<TableCell>
									<div className="flex flex-wrap gap-1">
										{membership.accessCategories.map(
											(cat) => (
												<span
													key={cat}
