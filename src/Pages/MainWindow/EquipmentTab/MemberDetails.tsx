...
export default function MemberDetails({userDetails}: any ) {
  ...
  // Conditional rendering if no user is selected
  if (!userDetails) {
    return (
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center space-x-4">
          <Avatar className="h-16 w-16">
            {/* <AvatarImage src={userDetails.avatarUrl} alt={`${userDetails.firstName} ${userDetails.lastName}`} /> */}
            {/* <AvatarFallback>{userDetails.firstName}{userDetails.lastName}</AvatarFallback> */}
          </Avatar>
          <div>
            <h3 className="text-xl font-semibold">{userDetails.firstName} {userDetails.lastName}</h3>
            <p className="text-sm text-muted-foreground">Student ID: {userDetails.studentId}</p>
          </div>
        </div>
      </div>
          ...
          {equipmentList.map((equipment) => (
            ...
            >
              ...
              <div className="flex-grow">
                ...
                <p className="text-sm text-muted-foreground">Equipment ID: {equipment.id}</p>
              </div>
              <Button 
                variant="outline" 
                size="sm"
